...
async def get_ticket_overview(user_info: dict = Depends(require_authenticated_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get ticket overview: {str(e)}"
        )
...
async def get_owned_tickets(user_info: dict = Depends(require_authenticated_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get owned tickets: {str(e)}"
        )


@router.get("/{ticket_id}", summary="Get specific ticket details")
async def get_ticket_details(
    ticket_id: int, user_info: dict = Depends(require_authenticated_user)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get ticket details: {str(e)}"
        )


@router.get("/{ticket_id}/check-used", summary="Check if ticket is used")
async def check_ticket_used(
    ticket_id: int, user_info: dict = Depends(require_authenticated_user)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to check ticket status: {str(e)}"
        )
...
async def mark_ticket_used(
    ...
    """Mark a ticket as used - Only admin can do this"""
    try:
        user_address = user_info["wallet_address"]
        ...
        if "admin" not in user_roles:
            raise HTTPException(
                status_code=403, detail="Only admin can mark tickets as used"
            )
        ...
        if is_used:
            raise HTTPException(
                status_code=400, detail="Ticket is already marked as used"
            )

        # Mark ticket as used using oracle account (the predefined admin account 0)
        admin_account = web3_manager.get_user_account_by_index(
            0
        )  # Use the hardhat account 0 as oracle
        ...
        txn = web3_manager.build_user_transaction(
            function_call, admin_account, gas=150000
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to mark ticket as used: {str(e)}"
        )
